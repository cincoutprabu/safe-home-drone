...
-(NSString *) calcValue:(NSData *) value {
    ...
    if (!lightArray) lightArray = [NSMutableArray new];
    if (lightArray.count == 5) [lightArray removeObjectAtIndex:0];
    [lightArray addObject:[NSNumber numberWithFloat:self.lightLevel]];
    float averageLight = [self getAverageLightLevel];
    if (averageLight <= 2.0) {
        //ViewController *vc = (ViewController*)[UIApplication sharedApplication].keyWindow.rootViewController;
        //[vc takeOffButtonTouched:nil];
    }
    
    self.lightLevel = (float)[sensorTagLightService sfloatExp2ToDouble:dat];
    NSLog(@"Light: %0.1f, Average: %0.1f", self.lightLevel, [self getAverageLightLevel]);
...
{
    float sum = 0.0;
    for (NSNumber *num in lightArray) {
        sum += [num floatValue];
    }
    return sum / lightArray.count;
}
